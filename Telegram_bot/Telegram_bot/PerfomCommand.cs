...
namespace Telegram
{
                ...
                while (true)
                {
                    using (WebClient webClient = new WebClient())
                    {
                        string response = webClient.DownloadString
                            ("https://api.telegram.org/bot" + _token + "/getupdates?offset=" + (LastUpdateID + 1));
                        ...
                        foreach (JSONNode r in N["result"].AsArray)
                        {
                            ...
                            e.name = r["message"]["from"]["first_name"] + r["message"]["from"]["last_name"];
    ...
    class Method
    {
        ...
        public void FormatMessage(int chatId, int fromChatID, int messageID)
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
        ...
        async public Task SendPhotoImputFile(int ChatID, string pathToPhoto, string catprion = "")
        {
